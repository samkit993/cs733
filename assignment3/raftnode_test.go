...
func TestBasic(t *testing.T){
	rand.Seed(3007) 
	registerStructs()
	raft1, err := NewRaftNode(FOLLOWER, 1, "input_spec.json", "log1", 150, 350)
	checkError(t,err)
	raft2, err := NewRaftNode(FOLLOWER, 2, "input_spec.json", "log2", 150, 350)
	checkError(t,err)
	raft3, err := NewRaftNode(FOLLOWER, 3, "input_spec.json", "log3", 150, 350)
	checkError(t,err)
	raft4, err := NewRaftNode(FOLLOWER, 4, "input_spec.json", "log4", 150, 350)
	checkError(t,err)
	raft5, err := NewRaftNode(FOLLOWER, 5, "input_spec.json", "log5", 150, 350)
	checkError(t,err)
	raft6, err := NewRaftNode(FOLLOWER, 6, "input_spec.json", "log6", 150, 350)
	checkError(t,err)
